...
function extractSubagentTypes(content: ContentBlock[]): string[] {
  return content
    .filter((b): b is ToolUseBlock => b.type === 'tool_use' && (b as ToolUseBlock).name === 'Agent')
    .map(b => {
      const t = (b.input as Record<string, unknown>)?.subagent_type
      return typeof t === 'string' && t.trim() ? t.trim() : 'general-purpose'
    })
}

function extractSkills(content: ContentBlock[]): string[] {
  return content
    .filter((b): b is ToolUseBlock => b.type === 'tool_use' && (b as ToolUseBlock).name === 'Skill')
    .map(b => {
      const s = (b.input as Record<string, unknown>)?.skill
...
function extractFiles(content: ContentBlock[]): string[] {
  const out: string[] = []
  for (const b of content) {
    if (b.type !== 'tool_use') continue
    const tu = b as ToolUseBlock
    if (!FILE_TOOLS.has(tu.name)) continue
...
function parseApiCall(entry: JournalEntry): ParsedApiCall | null {
  ...
  const bashCmds = extractBashCommandsFromContent(content)
  ...
  return {
    ...
    hasTodoWrite: tools.includes('TodoWrite'),
    subagentTypes: extractSubagentTypes(content),
    skills: extractSkills(content),
    files: extractFiles(content),
    gitBranch: typeof entry.gitBranch === 'string' ? entry.gitBranch : null,
    ccVersion: typeof entry.version === 'string' ? entry.version : null,
