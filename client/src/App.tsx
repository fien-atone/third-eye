...
type ProjectInfo = { id: string; key: string; label: string; calls: number; cost: number; firstTs: string; lastTs: string }
...
/** Compact stable-width date for table cells — year always aligns at the right when column is right-aligned. */
function useFmtDateCompact() {
  const dl = useDateLocale()
  return (iso: string) => format(parseISO(iso), 'yyyy-MM-dd', { locale: dl })
}
...
export default function App() {
  ...
  const overviewQuery = useQuery<OverviewResponse>({
    ...
    queryFn: async () => {
      const params = new URLSearchParams({
        ...
        weekStartsOn: String(weekStartsOn),
      })
  ...
  const refreshMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!r.ok) throw new Error(await r.text())
      return r.json() as Promise<{ ok: boolean; durationMs: number; total: number }>
    },
    onSuccess: () => {
  ...
  const insightsQuery = useQuery<InsightsResponse>({
    ...
    queryFn: async () => {
      ...
      const r = await fetch(`/api/insights/${projectId}?${params}`)
      if (!r.ok) throw new Error(await r.text())
      return r.json()
    },
...
function VersionsPanel({ rows }: { rows: VersionRow[] }) {
  const t = useT()
  const fmtDateCompact = useFmtDateCompact()
                ...
                {sorted.map((v, i) => {
                  ...
                  return (
                    <tr key={v.name}>
                      ...
                      <td className="num" style={{ color: 'var(--text-dim)', fontVariantNumeric: 'tabular-nums' }}>{fmtDateCompact(v.first_ts)}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      <div className="grid col-2">
        ...
        {!inProjectView && (
          <div className="panel">
            ...
            <table className="breakdown">
              <thead><tr><th>{t('panel.topProjects.colProject')}</th><th className="num">{t('panel.topProjects.colCalls')}</th><th className="num">{t('panel.topProjects.colCost')}</th></tr></thead>
              <tbody>
